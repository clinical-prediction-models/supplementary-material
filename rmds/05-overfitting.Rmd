...
plot(x=seq(from=-.025,to=.975,by=.05), dbinom(x=0:20,20,mort), axes=F,  type = "s", lwd=2,
...
text(x=1, y=.3, ifelse(sdtau==0,"No heterogeneity", 
        ifelse(sdtau!=0, paste("+/-", sdtau))), cex=1, adj=1, font=2)
for (i in (1:ncenter)) {
 set.seed(i+seedn)
 lines(x=c(0+runif(1,min=-.07,max=.07),1), 
       y=c(mortmat[i,2]+runif(1,min=-.001,max=.01), mortmat[i,3]), col=mycolors[rep(1:10,10)[i]])
 set.seed(i+seedn)
 points(x=c(0+runif(1,min=-.07,max=.07),1), 
