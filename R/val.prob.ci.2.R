...
#' val.prob.ci.2(Pred, yval,
#'   CL.smooth = TRUE, logistic.cal = TRUE, lty.log = 2,
#'   col.log = "red", lwd.log = 1.5
#' )
#'
#' val.prob.ci.2(Pred, yval,
#'   CL.smooth = TRUE, logistic.cal = TRUE, lty.log = 9,
...
val.prob.ci.2 <- function(p, y, logit, group, weights = rep(1, length(y)), normwt = F, pl = T,
  ...
  if (length(p) > 5000 & smooth == "loess") {
    warning("Number of observations > 5000, RCS is recommended.", immediate. = T)
  }
  if (length(p) > 1000 & CL.BT == T) {
    warning("Number of observations is > 1000, this could take a while...", immediate. = T)
  }
  ...
  if (pl) {
    plot(0.5, 0.5,
      xlim = xlim, ylim = ylim, type = "n", xlab = xlab,
      ylab = ylab, las = las, ...
    )
    clip(0, 1, 0, 1)
      ...
      if (CL.smooth == T | CL.smooth == "fill") {
        ...
        if (CL.BT == T) {
          ...
          if (CL.smooth == "fill") {
            ...
            clip(0, 1, 0, 1)
            lines(to.pred, CL.BT[1, ], lty = 2, lwd = 1, col = col.smooth)
            clip(0, 1, 0, 1)
            lines(to.pred, CL.BT[2, ], lty = 2, lwd = 1, col = col.smooth)
            do.call("clip", as.list(par()$usr))
    ...
    if (smooth == "rcs") {
      ...
      if (nr.knots == 5) {
        tryCatch(rcspline.plot(p, y,
          model = "logistic", nk = 5, show = "prob", statloc = "none", noprint = T,
          ...
          tryCatch(rcspline.plot(p, y,
            model = "logistic", nk = 4, show = "prob", statloc = "none", noprint = T,
            add = T, showknots = F, xrange = c(min(na.omit(p)), max(na.omit(p))), lty = lty.smooth
          ),
          error = function(e) {
            warning("Nk 4 also led to estimation problems, nk will be set to 3.", immediate. = T)
            rcspline.plot(p, y,
              model = "logistic", nk = 3, show = "prob", statloc = "none", noprint = T,
        ...
        stop(paste("Number of knots = ", nr.knots, sep = "", ", only 5 >= nk >=3 is allowed."))
      }
  ...
  Bscaled <- 1 - B / Bmax
  stats <- c(
    Dxy, C, R2, D, lr, p.lr, U, U.chisq, p.U, Q, B,
  ...
  if (pl) {
    ...
    if (!is.logical(lp)) {
      ...
      legend(lp, leg,
        lty = lt, pch = marks, cex = cex.leg, bty = "n", lwd = lw.d,
    ...
    if (!is.logical(statloc)) {
      if (dostats[1] == T) {
        ...
        leg <- paste(leg, ":", format(stats[dostats], digits = roundstats),
          sep =
            ""
        )
        if (!is.list(statloc)) {
        ...
        text(statloc$x + (xlim[2] - xlim[1]) / 3, statloc$y, paste(
          format(round(stats[dostats], digits = roundstats)),
          collapse =
            "\n"
        ), adj = 1, cex = cex)
      }
